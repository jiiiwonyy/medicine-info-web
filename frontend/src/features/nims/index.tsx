...
const GUIDE_ITEMS = [
  ...
  { icon: '📖', label: '매뉴얼', desc: '마약류 취급보고, 수출입, 제조/사용자, 연구자 보고' },
  ...
  return (
    <PageLayout title="마약류통합관리시스템(NIMS)">
      <Callout variant="info" className="mb-10">
        <span className="font-semibold">마약류의 제조, 수입, 유통, 사용, 폐기</span>까지의
        전 과정을 실시간으로 보고·관리하는 국가 통합 플랫폼입니다.
        보건복지부와 식품의약품안전처가 운영하며, 마약류의 오남용 방지,
        의료기관의 책임 있는 사용 관리, 국가 차원의 유통 투명성 확보를 목표로 합니다.
      </Callout>
      ...
      <section className="mb-10">
        ...
        <div className="flex flex-col gap-3">
          {FEATURES.map((f) => (
            <Card key={f.title} variant="outlined" padding="md" className="flex items-start gap-4">
      ...
      <section className="mb-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {GUIDE_ITEMS.map((item) => (
            <Card key={item.label} variant="muted" padding="md" className="flex items-start gap-3">
              <span className="text-xl shrink-0">{item.icon}</span>
              <div>
                <p className={cn(textStyles.titleSm, 'mb-0.5')}>{item.label}</p>
                <p className={cn(textStyles.bodyMd, 'text-muted-fg')}>{item.desc}</p>
              </div>
            </Card>
          ))}
        </div>
      ...
      <Button asChild className="w-full">
        <a href="https://www.nims.or.kr/" target="_blank" rel="noopener noreferrer">
